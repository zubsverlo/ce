...
//            var searchText = document.createTextNode((obj.options.text.search) + ': ');
            ...
            obj.searchInput.placeholder = "Поиск..."
            obj.searchInput.classList.add('jexcel_search');
            obj.searchInput.classList.add('form-control');
...
//            filter.appendChild(paginationUpdateContainer);
            ...
            if (obj.options.myOwnToolbar) {
                obj.ownToolbar = document.createElement('div')
                obj.toolbar_btns = document.createElement('div')
                obj.toolbar_btns.id = "toolbar_btns"
                obj.toolbar_btns.classList.add('col')
                obj.ownToolbar.classList.add('row')
                obj.ownToolbar.style.marginLeft = "auto";
                searchContainer.classList.add('col-3')
                searchContainer.classList.add('align-self-end')
//                searchContainer.classList.add('form-control')
                obj.ownToolbar.appendChild(obj.toolbar_btns)
                obj.ownToolbar.appendChild(searchContainer)
                el.appendChild(obj.ownToolbar)
            }
            else {
        ...
        */
        ...
        /**
         * Remerge first column
         * arneyK
         */
        obj.reMergeFirstColumn = function(){
            obj.destroyMerged();
            var ffill = obj.getColumnData(0);
            var s_value = ffill[0]
            var i = 0;
            for (i=0; i<ffill.length; i++) {
                if (ffill[i] === "") {
                    ffill[i] = s_value;
                }
                else {
            ...
            for (i=0; i<=ffill.length; i++) {
                var v = ffill[i];
                if (v !== s_v) {
                    if (i-s_i > 1) to_merge.push(['A'+String(s_i+1), i-s_i]);
                    s_v = v;
                    s_i = i;
                };
                e_i = i+1;
            };
            if (e_i-s_i > 1) to_merge.push(['A'+String(s_i+1), e_i-s_i]);
            for (i in to_merge) obj.setMerge(to_merge[i][0], 0, to_merge[i][1]);
        }
...
//                    }
    ...
    jexcel.timeControlLoading = null;
    // arneyK
    jexcel.LenIterator = class {
        constructor(len, start=0) {
            this.start = start;
            this.current = start;
            this.len = len;
        }
        ...
        if (jexcel.current) {
            ...
            if (! jexcel.current.edition && jexcel.current.selectedCell) {
                ...
                if (e.which == 37) {
                    ...
                    if (jexcel.current.contextMenu.contextmenu.isOpened()) {
                        var curr_id = jexcel.current.contextMenu.contextmenu.lenIterator.current
                        var new_id = jexcel.current.contextMenu.contextmenu.lenIterator.prev()
                        $('#option_'+curr_id).removeClass('option_selected')
                        $('#option_'+new_id).addClass('option_selected')
                        e.preventDefault();
                    }
                    else {
                    ...
                    if (jexcel.current.contextMenu.contextmenu.isOpened()) {
                        var curr_id = jexcel.current.contextMenu.contextmenu.lenIterator.current
                        var new_id = jexcel.current.contextMenu.contextmenu.lenIterator.next()
                        $('#option_'+curr_id).removeClass('option_selected')
                        $('#option_'+new_id).addClass('option_selected')
                        e.preventDefault();
                    }
                    ...
                    else if (jexcel.current.contextMenu.contextmenu.isOpened())
                    {
                        $('.option_selected')[0].method()
                    ...
                    if ((e.ctrlKey || e.metaKey) && ! e.shiftKey) {
                        if (e.which == 65) {
                            ...
                            if (jexcel.current.options.editable == true) {
                                ...
                                x_custom = exp.getAttribute('data-x')
                                y_custom = exp.getAttribute('data-y')
                                jexcel.contextMenuControls()
                                x_custom = null;
                                y_custom = null;
                            }
                            e.preventDefault();
...
//                                    console.log(String.fromCharCode(e.keyCode) == e.key, String.fromCharCode(e.keyCode).toLowerCase() == e.key.toLowerCase(), String.fromCharCode(e.keyCode).toLowerCase(), e.key, jexcel.validLetter(String.fromCharCode(e.keyCode)))
                                    ...
                                    if (e.keyCode == 32) {
                                        ...
                                        jexcel.current.openEditor(jexcel.current.records[rowId][columnId], false);
                                               ...
                                               ((String.fromCharCode(e.keyCode) == e.key || jexcel.validLetter(String.fromCharCode(e.keyCode))))) {
                                        // Start edition
                                        jexcel.current.openEditor(jexcel.current.records[rowId][columnId], true);
    ...
    jexcel.contextMenuControls = function(e) {
        ...
        if (jexcel.current) {
            if (jexcel.current.edition) {
                ...
                jexcel.current.contextMenu.contextmenu.close();
                jexcel.current.contextMenu.contextmenu.lenIterator = null
                if (jexcel.current) {
                    ...
                    if (x || y) {
                        ...
                        if (items.length) {
                            var lenIterator = new jexcel.LenIterator(items.length, 1)
                            jexcel.current.contextMenu.contextmenu.lenIterator = lenIterator
                        }

                        jexcel.current.contextMenu.contextmenu.open(e, items);
                        $('.jcontextmenu-focus').children().each(function(i, obj) {
                            obj.id = 'option_'+i
                        });
                        $('#option_1').addClass('option_selected')
                        // Avoid the real one
                        e.preventDefault();
